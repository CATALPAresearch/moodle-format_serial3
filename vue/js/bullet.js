...
(function () {
  d3.bullet = function () {
    ...
    function bullet(g) {
      g.each(function (d, i) {
        ...
        // Compute the new x-scale with fixed domain 0-100 for percentages
        var maxDomain = Math.max(rangez[0] || 0, measurez[0] || 0);
        // Use 100 as fixed max for percentage indicators, unless values exceed 100
        var domainMax = maxDomain > 100 ? maxDomain : 100;
        var x1 = d3.scaleLinear().domain([0, domainMax]).range([0, width]);
  ...
  function bulletWidth(x) {
    var x0 = x(0);
    return function (d) {
      var width = Math.abs(x(d) - x0);
      // Ensure minimum visible width of 2px for zero values
