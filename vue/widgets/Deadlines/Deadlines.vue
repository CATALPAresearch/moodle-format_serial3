<template>
  <div class="position-relative h-100 d-flex flex-column">
    <widget-heading icon="fa-calendar-o" :info-content="info" title="Termine">
    </widget-heading>
    ...
    <ul class="deadline-items flex-shrink-1 m-0 p-0" style="max-height: 100%">
      <li
        ...
        :class="{
          ...
          'mb-1': true,
          'mr-1': true,
          'border-today': isDueToday(deadline),
        }"
      >
        <a v-if="deadline.url" :href="deadline.url">
          <p class="mb-2">
            <span v-if="deadline.timestart">{{
              formatDate(deadline.timestart)
            }}</span>
            <span v-if="deadline.timeclose != deadline.timestart"
              >- {{ formatDate(deadline.timeclose) }}</span
            >
          </p>
          <i
            v-if="deadline.type === 'calendar'"
            aria-hidden="true"
            class="icon fa fa-calendar fa-fw"
          ></i>
          <img
...
export default {
  ...
  computed: {
    ...
    filteredDeadlines() {
      ...
      deadlines = deadlines.filter(
        (deadline) => deadline.timeclose > Date.now() / 1000,
      );
  ...
  methods: {
    ...
    async getAssignmentData() {
      const response = await Communication.webservice("get_deadlines", {
        courseid: this.$store.state.courseid,
      });
      if (response.success) {
        const data = JSON.parse(response.data);
        ...
        for (let key in this.assignments) {
          ...
          this.assignments[key].url =
            this.$store.getters["overview/getUrlById"](id);
        }
      } else {
        if (response.data) {
          console.error(
