    ...
    <div class="d-flex justify-content-between align-items-center">
      <div class="form-group form-check mt-2 ml-1">
        <input
          ...
          type="checkbox"
        />
        <label class="form-check-label" for="compare-to-average"
          >Vergleich mit Kurs</label
        >
      </div>
...
export default {
  ...
  methods: {
    ...
    async getQuizzes() {
      const response = await Communication.webservice("get_quizzes", {
    ...
    async getAssignments() {
      const response = await Communication.webservice("get_assignments", {
        course: this.$store.state.courseid,
      });
    ...
    drawChart() {
      ...
      var xDomain = [
        0,
        d3.max(this.data, (d) => Math.max(d.value, d.avg_value)),
      ];
      ...
      if (this.showAverage) {
        ...
        svg
          ...
          .attr(
            ...
            (d) =>
              yScale(d.category) +
              (yScale.bandwidth() - yScale.bandwidth() / 1.5) / 2,
          )
          .attr("width", (d) => xScale(d.value) - xRange[0])
